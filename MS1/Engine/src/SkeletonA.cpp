...
namespace Azul
{
	...
	SkeletonA::SkeletonA(Bone *pBone, int _numBones)
		: pFirstBone(nullptr),
		numBones(_numBones)
	{
	...
	GameObjectAnim *SkeletonA::GetFirstBone()
	{
		assert(pFirstBone);
		return this->pFirstBone;
	}

	GameObjectAnim *SkeletonA::privFindBoneByIndex(int index)
	{
	...
	void SkeletonA::privSetAnimationHierarchy(Bone *pBoneResult)
	{
		assert(pBoneResult);

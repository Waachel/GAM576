...
namespace Azul
{
		...
		numBones(NUM_BONES)
	{
		...
		this->poSkeleton = new SkeletonA(this->poBoneResult, numBones);
	...
	AnimTime AnimA::FindMaxTime()
	{
		assert(pClip);
		return this->pClip->GetTotalTime();
	}

	void AnimA::Animate(AnimTime tCurr)
	{
		this->pClip->AnimateBones(tCurr, this->poBoneResult);
	}


	void AnimA::PoseSkeleton(GameObjectAnimSkeleton *node)
	{
		// safety
		assert(node);
